...
def pull_market_data():
    ...
    # ── S&P 500 3-Year Daily Chart History ──
    try:
        print(f"  📈 Pulling S&P 500 3-year daily history for chart...")
        sp_chart = yf.Ticker("^GSPC")
        chart_end = datetime.today()
        chart_start = chart_end - timedelta(days=3*365)
        sp_chart_hist = sp_chart.history(start=chart_start, end=chart_end, interval="1d")
        
        if not sp_chart_hist.empty:
            market["sp500_daily_prices"] = [round(float(c), 2) for c in sp_chart_hist["Close"].tolist()]
            market["sp500_daily_dates"] = [d.strftime("%Y-%m-%d") for d in sp_chart_hist.index]
            print(f"  ✅ S&P 500 chart: {len(market['sp500_daily_prices'])} daily closes "
                  f"({market['sp500_daily_dates'][0]} to {market['sp500_daily_dates'][-1]})")
        else:
            print(f"  ❌ S&P 500 chart: No data returned")
    except Exception as e:
        print(f"  ❌ S&P 500 chart history: {e}")
